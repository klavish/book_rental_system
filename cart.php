...
<li class="flex gap-2 justify-center items-center">
    <div class="relative">
        <a href="">
            ...
            <span class="px-1 w-6 h-6 absolute -top-3 -right-1 flex items-center justify-center rounded-full   text-white">
                <?php 
                //if (isset($_SESSION['loginUser']) && ($_SESSION['loginUser']['book'])) { 
                // ($_SESSION['loginUser'][0]['userId']);
                // $object = new Database();
                //$object->sql("select count(bookId) from rentedbooks where userId =''"); 
                //}
                 ?></span>
    </div>
    </a>
